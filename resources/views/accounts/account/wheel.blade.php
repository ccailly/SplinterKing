<x-table title="Historique" badge="{{ $wheels['total'] }} roulettes" :columns="$wheels['columns']" :rows="$wheels['rows']"
    :search="$wheels['search']" :pagination="$wheels['pagination']">
    <x-slot name="empty">
        <x-slot name="buttons">
            @if ($wheels['available'])
        ...
        @if ($wheels['search']['value'] != '')
            <x-empty-table title="Aucune roulette trouvée"
                desc="Vous avez recherché <strong>{{ $wheels['search']['value'] }}</strong>, ce qui ne correspond a aucune roulette. Veuillez réessayer en étant plus permissif.">
                <x-slot name="icon">
        ...
        @else
            <x-empty-table title="Aucune roulette trouvée" desc="Aucune roulette n'a encore été tourné sur ce compte.">
                <x-slot name="icon">
